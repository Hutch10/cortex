...
* `package identity == manifest identity`? **UNRESOLVED/NOT_REQUIRED**. If a package needs no identity, the manifest's content hash identifies the manifest itself.
* `manifest digest == manifest identity`? **YES**. Content-addressing is the safest identification for static artifacts.
* `package digest == package identity`? **YES**. Content-addressing perfectly identifies the physical package.
...
* **C.** Exact copy preserve identity? **YES** (if content-addressed, bytes are identical).
...
* **G.** Changing compression preserve identity? **NO** (if content-addressed package bytes change) or **YES** (if we only care about logical manifest digest).
* **H.** Manifest regeneration create new package identity? **YES** (new bytes).
* **I.** Re-export same state create new package identity? **YES** (timestamps or new signatures change bytes).
* **J.** Re-export after unrelated entry disposed create new package identity? **YES**.
* **K.** Partial export require own package identity? **NO** (it just has a different manifest digest).
* **L.** Two packages contain same entry set but different identities? **YES** (exported at different times).
* **M.** Two packages same identity but differing bytes? **NO** (impossible if content-addressed; if random ID, yes, which is dangerous).
* **N.** Two manifests describe same package? **YES** (e.g., different format manifests).
* **O.** One manifest describe multiple packages? **NO**.
* **P.** Import preserve source package identity? **YES** (the source package digest remains static).
* **Q.** Clone preserve source package identity? **NO** (clones generate new packages).
* **R.** Merge preserve source package identities? **YES** (as provenance).
* **S.** Merged output receive new package identity? **YES** (it is a new set of bytes).
* **T.** Inferred from archive identity + timestamp? **NO** (partial exports can occur simultaneously).
* **U.** Inferred from manifest digest? **YES**.
* **V.** Content-addressed? **YES**.
* **W.** Unsupported future structures retain identity? **YES** (digests don't require semantic parsing).
...
* **MODEL A — No package identity**: Packages are identified only by their content digests. Lineage exists between entries, not packages.
* **MODEL B — Random portable package identity**: Rejected. Acts as a tracking identifier and leaks export cadence.
* **MODEL C — Content-addressed package identity**: The canonical package digest or manifest digest serves as the only identity.
...
If P1 is copied byte-for-byte, it remains physically identical to P1 (same digest). Vitalicast does not know a copy occurred, nor should it. A copy is a custody/transport event, not an architectural identity event.
...
When P1 is imported, its manifest and entries are ingested. P1's identity (its digest) is preserved as provenance. The import event does not "become" the archive identity. Clone operations initialize from entries, not packages; future exports from the clone are entirely new packages.

## 10. Merge Semantics
Merging Archive A and Archive B results in a new current inventory. The source manifests from A and B may be retained as Grade D provenance (identified by their digests), but the merged package has no random identity. No "package lineage" graph is generated.
...
* Content digests (integrity attestations) are sufficient to identify physical packages and manifests.
